...
//console.log
    ...
    // Get Profile Picture Group
       ...
       conn.send5ButImg(azka.id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, Azka-MD, [])
       } else if (dani.announce == false) {
       conn.send5ButImg(azka.id, `「 Group Settings Change 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message`, Azka-MD, [])
       } else if (dani.restrict == true) {
       conn.send5ButImg(azka.id, `「 Group Settings Change 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message`, Azka-MD, [])
       } else if (dani.restrict == false) {
       conn.send5ButImg(azka.id, `「 Group Settings Change 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message`, Azka-MD, [])
       } else {
       conn.send5ButImg(azka.id, `「 Group Settings Change 」\n\nGroup Subject telah diganti menjadi *${Azka.subject}*`, `Group Settings Change Message`, Azka-MD, [])
     }
    }
    } catch (err){
    console.log(err)
